...
#define OXIMETER_CS  RPI_V2_GPIO_P1_7
...
int main(int argc, char **argv) {
   ...
   //   bcm2835_spi_setChipSelectPolarity(OXIMETER_CS, LOW);      // the default

	//set the OXIMETER_CS to be an output
	//bcm2835_gpio_fsel(OXIMETER_CS, BCM2835_GPIO_FSEL_OUTP);

	bcm2835_gpio_write(OXIMETER_CS,HIGH);

	
	uint8_t mosi_Start_Read_OXIMETER_Measurement_Process[1] = {0x0A};
	uint8_t miso_trigger_OXIMETER_Measurement[1] = {0x0};
	
	uint8_t miso_copy_OXIMETER_Data[2];
	uint8_t mosi_trigger_OXIMETER_Data[2] = {0x14,0x14};
...
uint8_t miso_receive_ack_for_first_call_for_data[1] ;
	...
	bcm2835_gpio_write(OXIMETER_CS,LOW);
	...
	//for (char ret = 0; ret < 12; ret++) {
		//transfer here
		miso_trigger_OXIMETER_Measurement[0] = bcm2835_spi_transfer(mosi_Start_Read_OXIMETER_Measurement_Process[0]);
	//}
...
//	bcm2835_spi_end();
	...
	//printf("%d",miso_trigger_OXIMETER_Measurement[0]);
	//printf("\n");

	if((int)miso_trigger_OXIMETER_Measurement[0]==0x0F){
...
// this is for the blood pressure meter or any other sensor to give time for the measurement to complete
	...
	bcm2835_delayMicroseconds(.777);
		...
		bcm2835_spi_transfernb(miso_copy_OXIMETER_Data,mosi_trigger_OXIMETER_Data,2);
